...
function addPassesToPassLibrary() {
    ...
    passes[0] = aPass1;
    passes[1] = aPass2;
    addPassesWithPath(passes, true, function() {
...
function removePassFromLibrary() {
    ...
    addPassWithPath(path, true, function() {
        ...
        passLibrary.removePass(aPass);
        if (!passLibrary.containsPass(aPass)) alert("Removed Pass From Library");
        else alert("Pass still exists in Passbook.\n Couldn't able to remove Pass.");
    });
}
...
function removeMissingPassFromLibrary() {
    ...
    var aPass = createPassFromFile(couponPassPath);
    passLibrary.removePass(aPass);
    if (!passLibrary.containsPass(aPass)) alert("No Errors , if we tried to remove an already removed pass");
    else alert("Pass still exists in Passbook.\n Couldn't able to remove Pass.");
}
/*
 *
...
function addPassWithPath(path, isAnimated, verificationCallback) {
    ...
    if (passLibrary.containsPass(aPass) == false) {
        ...
        passLibrary.addPassWithCompletionCallback(aPass, function(addPassStatus) {
            if (addPassStatus == "KonyPKPassLibraryDidAddPasses") {} else if (addPassStatus == "KonyPKPassLibraryShouldReviewPasses") {
        ...
        function callVerification() {
            if (flag) {
                flag = false;
                if (typeof verificationCallback === 'function') verificationCallback();
            }
            try {
                kony.timer.schedule(new Date().getTime().toString(), callVerification, 2, false);
            } catch (err) {}
        }
    } else if (typeof verificationCallback === 'function') alert("Pass you are trying add is already exists in the Passbook");
}
/*
 * 
 * 
...
function addPassesWithPath(passes, isAnimated, verificationCallback) {
    ...
    } else if (typeof verificationCallback === 'function') alert("Passes trying to add are already exists in the Passbook");
    //verificationCallback();
}
...
function isFileExists(filepath) {
    ...
    file = myfile;
    if (myfile.exists()) alert("File Exists");
    else alert("No File Exists");
}